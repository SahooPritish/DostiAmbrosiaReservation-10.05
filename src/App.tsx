...
function BookTab({ onSubmit, existingBookings, lastSubmission, addToast, tick }: { 
  ...
  // Keep Player 1 in sync with Resident Name
  useEffect(() => {
    setForm(f => {
      const names = [...f.playerNames];
      names[0] = f.residentName;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isValid) return;
    const players = form.playerNames.map((name, i) => ({
      slot: i + 1,
      name: name.trim() || (i === 0 ? form.residentName : `Guest ${i + 1}`)
    }));
    ...
    <div className="space-y-8">
      {lastSubmission ? (
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 space-y-6">
          ...
          {/* Number of Players & Dynamic Names */}
          <div className="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label htmlFor="player-count" className="text-sm font-semibold uppercase tracking-wider text-slate-500">Number of Players</label>
                {form.playerCount >= 8 && (
                  <span className="text-[10px] font-bold text-amber-600 uppercase">Maximum 8 players per booking</span>
                )}
              </div>
              <div className="flex items-center gap-4">
                <button 
                  type="button" 
                  onClick={() => handlePlayerCountChange(Math.max(1, form.playerCount - 1))}
                ...
                <input
                  ...
                  onChange={e => handlePlayerCountChange(Math.min(8, Math.max(1, parseInt(e.target.value) || 1)))}
                  className="w-12 text-center bg-transparent font-bold text-lg outline-none"
                />
                <button 
                  type="button" 
                  onClick={() => handlePlayerCountChange(Math.min(8, form.playerCount + 1))}
                  className="w-10 h-10 rounded-full border border-slate-200 dark:border-slate-800 flex items-center justify-center font-bold"
                >+</button>
              </div>
            </div>
            ...
            <div className="space-y-3 pl-2 border-l-2 border-slate-100 dark:border-slate-800">
              {form.playerNames.map((name, i) => (
                <div key={i} className="space-y-1">
                  <label htmlFor={`player-${i + 1}`} className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                    {i === 0 ? 'Player 1 (You)' : `Player ${i + 1}`}
                  </label>
                  <input
                    id={`player-${i + 1}`}
                    type="text"
                    value={name}
                    onChange={e => {
                      const newNames = [...form.playerNames];
                      newNames[i] = e.target.value;
                      setForm(f => ({ ...f, playerNames: newNames }));
                    }}
                    placeholder={i === 0 ? form.residentName : `Guest ${i + 1}`}
                    className={`w-full text-sm rounded-lg px-3 py-2 outline-none border transition-all ${
                      i === 0 
                        ? 'bg-slate-50 dark:bg-slate-900 border-dashed border-slate-300 dark:border-slate-700 font-medium' 
                        : 'bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus:ring-1 focus:ring-primary focus:border-primary'
                    }`}
                  />
                </div>
              ))}
...
function PaymentDisclaimer({ booking }: { booking: Booking }) {
  ...
  return (
    <motion.div
      ...
      className="mt-4 p-4 md:p-5 bg-[oklch(0.92_0.12_85)] dark:bg-[oklch(0.28_0.08_75)] border border-[oklch(0.80_0.14_85)] dark:border-[oklch(0.42_0.10_78)] rounded-xl space-y-3"
    >
      <div className="flex items-center gap-2 text-[oklch(0.35_0.12_60)] dark:text-[oklch(0.88_0.10_85)]">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
          <line x1="12" y1="9" x2="12" y2="13"/>
          <line x1="12" y1="17" x2="12.01" y2="17"/>
        </svg>
        <span className="font-bold">Slot Reserved — Payment Required</span>
      </div>
      <p className="text-sm text-[oklch(0.40_0.10_65)] dark:text-[oklch(0.88_0.10_85)/80]">
        If payment is not received within 30 minutes, the slot will be re-opened for other residents.
      </p>
      <p className="text-xs text-[oklch(0.45_0.08_65)] dark:text-[oklch(0.88_0.10_85)/60]">
        Please pay via UPI or Cash immediately to confirm your booking.
      </p>
      <div className="flex justify-start sm:justify-end">
        <span className="font-bold bg-[oklch(0.85_0.14_82)] dark:bg-[oklch(0.38_0.10_78)] text-[oklch(0.35_0.12_60)] dark:text-[oklch(0.88_0.10_85)] rounded-full px-3 py-1 text-sm">
          Total Due: ₹{booking.totalAmount}
        </span>
      </div>
    </motion.div>
  );
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-y-2 text-sm">
                  ...
                  <span className="text-right text-xs">
                    <span className="font-bold">{b.playerCount}</span> — {b.players.map(p => p.name).join(', ')}
                  </span>
                  ...
                  <div className="p-4 space-y-3">
                    <div className="flex justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-medium">Players: {b.playerCount} — {b.players.map(p => p.name).join(', ')}</p>
                      </div>
...
function RulesTab() {
  const sections = [
    ...
    { title: "5. AUTO-CANCELLATION POLICY", items: [
      "1. Once a booking is made, payment must be completed within 30 minutes to secure your slot.",
      "2. If payment is not received within 30 minutes, the slot will be automatically re-opened for other residents.",
      "3. Cancelled bookings cannot be reinstated. A fresh booking must be made if the slot is still available.",
      "4. Payment can be made via UPI or Cash to the facility management team."
    ] },
    { title: "6. BOOKING MODIFICATIONS", items: [
      "1. You can change the date or start time of a confirmed booking, provided the slot is more than 2 hours away.",
      "2. Sport, duration, guest status, paddle count, and total amount cannot be changed after booking.",
      "3. No additional payment is required for modifications.",
      "4. Modifications are locked within 2 hours of your slot and cannot be made after that."
    ] },
    { title: "7. Dress Code and Equipment", items: ["Non-marking sports footwear is MANDATORY", "Studded or inappropriate footwear is STRICTLY PROHIBITED", "Users must bring their own equipment", "Pickleball paddles available for rent"] },
    { title: "8. Court Usage Guidelines", items: ["Vacate the court immediately after slot completion", "Net is multi-purpose and adjustable", "Coaching / training requires prior management approval"] },
    { title: "9. Code of Conduct", items: ["Maintain discipline, decorum, and sportsmanship", "Loud, disruptive, or inappropriate behaviour is not allowed"] },
    { title: "10. Prohibited Activities", items: ["FOOD, SMOKING, AND ALCOHOL PROHIBITED", "Pets and unauthorised individuals not allowed in play area"] },
    { title: "11. Safety", items: ["Children below 5 years must be under adult supervision", "Exercise caution and follow safe playing practices"] },
    { title: "12. General Conditions", items: ["Follow Court Manager instructions", "Damage recovery from responsible person", "Management / Court Manager decisions are final and binding"] },
    { title: "13. Changes to Rules", items: ["These rules are subject to change as decided by Management", "Residents will be notified of updates", "Latest version displayed in app"] },
  ];
